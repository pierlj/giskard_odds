...
EMPIRE_SCHEMA = {
                    "countdown": int,
                    "bounty_hunters": [{"planet": str, "day": int}]
                }
...
def build_unvierse_graph(db_path, millenium_dict):
    ...
    try: 
        ...
        assert all([edge[0] is not None and edge[1] is not None and edge[0] != '' and edge[1] != '' and edge[2] > 0 for edge in db_answer])
        edge_list = ["{} {} {{'weight':{}}}".format(edge[0], edge[1], int(edge[2])) for edge in db_answer
                            if int(edge[2]) <= autonomy] 

        con.close()
        
    except:
        logger.warn('An issue occurred during the opening of the DB file, route graph was not created.')
        return None
    ...
    if G.number_of_nodes() < 50: # if the routes graph is not too large display it 
        ...
        plt.axis('off')
        legend_handles = [Line2D([0], [0], marker='o', color='w', label='Circle',markerfacecolor='g', markersize=10),
                          Line2D([0], [0], marker='o', color='w', label='Circle',markerfacecolor='r', markersize=10)]
    ...
    for key in schema:
        ...
        if type(schema[key]) == type:
            if type(dict_check[key]) != schema[key]:
                logger.info("Wrong type for property {}, invalid json schema.".format(key))
                return False
        elif type(schema[key]) == list:
            if not all([type(schema[key][0]) == type(elem) for elem in dict_check[key]]):
                logger.info("Wrong type for element in list of property {}, invalid json schema.".format(key))
                return False
            if type(schema[key][0]) == dict and not all([check_json_schema(elem, schema[key][0]) for elem in dict_check[key]]):
                return False
...
def prettify_path(path , itinerary):
    path_str = []
    for idx_planet , (planet, (arrival_day, departure_day)) in enumerate(zip(path, itinerary)):
        if idx_planet == 0:
            if departure_day - arrival_day == 0:
                path_str.append("Departure from {} on day {}.".format(planet, arrival_day))
            elif departure_day - arrival_day == 1:
                path_str.append("First stay on {} for 1 days.".format(planet))
            else:
                path_str.append("First stay on {} for {} days.".format(planet, departure_day - arrival_day))
        elif idx_planet == len(path) - 1:
            path_str.append("Travel from {} to {}, arrival on day {}.".format(previous_planet, planet, arrival_day))
            ...
            if departure_day - arrival_day == 1:
                ...
                path_str.append("Refuel on {} and wait {} days (including refueling day).".format(planet, departure_day - arrival_day))
